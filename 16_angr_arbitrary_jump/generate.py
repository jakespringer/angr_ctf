...
def generate(argv):
  ...
  template = open(os.path.join(os.path.dirname(os.path.realpath(__file__)), '16_angr_arbitrary_jump.c.templite'), 'r').read()
  c_code = Templite(template).render(description=description)

  with tempfile.NamedTemporaryFile(delete=False, suffix='.c') as temp:
    temp.write(c_code)
    temp.seek(0)
    os.system('gcc -fno-stack-protector -Wl,--section-start=.text=0x34343434 -m32 -o ' + output_file + ' ' + temp.name)
