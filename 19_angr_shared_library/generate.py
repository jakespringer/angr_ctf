...
def generate(argv):
  ...
  template = open(os.path.join(os.path.dirname(os.path.realpath(__file__)), '19_angr_shared_library_so.c.templite'), 'r').read()
  c_code = Templite(template).render(description=description)

  with tempfile.NamedTemporaryFile(delete=False, suffix='.c') as temp:
    temp.write(c_code)
    temp.seek(0)
    os.system('gcc -I' + os.path.dirname(os.path.realpath(__file__))  + ' -fno-stack-protector -fpic -m32 -c -o 19_angr_shared_library.o ' + temp.name)
    os.system('gcc -shared -m32 -o ' + os.path.join('/'.join(output_file.split('/')[0:-1]), 'lib' + output_file.split('/')[-1] + '.so') + ' 19_angr_shared_library.o')
  ...
  template = open(os.path.join(os.path.dirname(os.path.realpath(__file__)), '19_angr_shared_library.c.templite'), 'r').read()
