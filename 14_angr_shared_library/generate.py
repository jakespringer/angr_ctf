...
def generate(argv):
  ...
  with tempfile.NamedTemporaryFile(delete=False, suffix='.c') as temp:
    ...
    os.system('chmod -x ' + os.path.dirname(os.path.realpath(__file__)) + '/lib' + output_file.split('/')[-1] + '.so')
  ...
  with tempfile.NamedTemporaryFile(delete=False, suffix='.c') as temp:
    temp.write(c_code)
    temp.seek(0)
    os.system('gcc -m32 -I ' + os.path.dirname(os.path.realpath(__file__)) + ' -Xlinker -R' + os.path.dirname(os.path.realpath(__file__)) + ' -L' + os.path.dirname(os.path.realpath(__file__)) + ' -o ' + output_file + ' ' + temp.name + ' -l' + output_file.split('/')[-1])
