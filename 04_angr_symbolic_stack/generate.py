...
def generate(argv):
  ...
  template = open(os.path.join(os.path.dirname(os.path.realpath(__file__)), '04_angr_symbolic_stack.c.templite'), 'r').read()
  ...
  with tempfile.NamedTemporaryFile(delete=False, suffix='.c', mode='w') as temp:
    temp.write(c_code)
    temp.seek(0)
    os.system('gcc -fno-stack-protector -fno-pie -no-pie -m32 -o ' + output_file + ' ' + temp.name)
